...
// // Game questions with map coordinates, dates, and layer settings
// const GAME_QUESTIONS = [
//   {
//     id: 1,
//     question: "This region shows intense thermal anomalies in August 2023. Which location experienced massive wildfires?",
...
// // Component to handle map movements with animation
...
//     //   },
//     };
//     return layerConfigs[layerId] || layerConfigs.MODIS_Terra_CorrectedReflectance_TrueColor;
//   };

//   if (gameComplete) {
//     const percentage = ((score / GAME_QUESTIONS.length) * 100).toFixed(0);
//     return (
...
//             <div className="score-label">out of {GAME_QUESTIONS.length}</div>
//             <div className="percentage">{percentage}% Accuracy</div>
//           </div>
...
//                 <span className="badge-text">NASA Expert!</span>
//               </>
//             )}
//             {percentage >= 70 && percentage < 90 && (
//               <>
//                 <span className="badge-icon">🚀</span>
//                 <span className="badge-text">Space Cadet!</span>
//               </>
//             )}
//             {percentage >= 50 && percentage < 70 && (
//               <>
//                 <span className="badge-icon">🛸</span>
//                 <span className="badge-text">Explorer!</span>
//               </>
//             )}
//             {percentage < 50 && (
//               <>
//                 <span className="badge-icon">🌍</span>
//                 <span className="badge-text">Earth Observer!</span>
//               </>
//             )}
//           </div>
...
//             </div>
//             <div className="stat-card">
//               <div className="stat-value">{GAME_QUESTIONS.length - score}</div>
//               <div className="stat-label">Incorrect</div>
//             </div>
//             <div className="stat-card">
//               <div className="stat-value">{GAME_QUESTIONS.length}</div>
...
//               style={{
//                 width: `${((currentQuestion + 1) / GAME_QUESTIONS.length) * 100}%`,
//               }}
//             />
//           </div>
...
//               <div className="transition-text">Flying to location...</div>
//             </div>
//           </div>
//         )}
//       </div>
...
//             {showResult && (
//               <div className={`result-inline ${answeredCorrectly ? 'correct-result' : 'incorrect-result'}`}>
...
//                 <button className="next-compact-button" onClick={handleNextQuestion}>
...
const Game = ({ onClose }) => {
  ...
  const getLayerConfig = (layerId) => {
    const layerConfigs = {
      MODIS_Terra_CorrectedReflectance_TrueColor: { matrixSet: "GoogleMapsCompatible_Level9", format: "jpg" },
      MODIS_Terra_CorrectedReflectance_Bands721: { matrixSet: "GoogleMapsCompatible_Level9", format: "jpg" },
      MODIS_Combined_Thermal_Anomalies_All: { matrixSet: "GoogleMapsCompatible_Level9", format: "png" },
      MODIS_Terra_Snow_Cover: { matrixSet: "GoogleMapsCompatible_Level8", format: "png" },
      MODIS_Terra_Land_Surface_Temp_Day: { matrixSet: "GoogleMapsCompatible_Level7", format: "png" },
      MODIS_Aqua_Chlorophyll_A: { matrixSet: "GoogleMapsCompatible_Level7", format: "png" },
      MODIS_Aqua_Aerosol_Optical_Depth: { matrixSet: "GoogleMapsCompatible_Level6", format: "png" },
    };
    return layerConfigs[layerId] || layerConfigs.MODIS_Terra_CorrectedReflectance_TrueColor;
  };
    ...
    <div className="game-container">
      ...
      <div className="game-map-fullscreen">
        ...
        >
          ...
          <TileLayer
            key={`base-${question.baseLayer}-${question.date}`}
            url={GIBS_URL(question.baseLayer, baseConfig.matrixSet, question.date, baseConfig.format)}
            attribution="&copy; NASA EOSDIS GIBS"
            noWrap={true}
            tileSize={256}
            maxNativeZoom={9}
          ...
          {question.layer && (
            <TileLayer
              key={`overlay-${question.layer}-${question.date}`}
              url={GIBS_URL(question.layer, overlayConfig.matrixSet, question.date, overlayConfig.format)}
              attribution="&copy; NASA EOSDIS GIBS"
              noWrap={true}
              tileSize={256}
              opacity={0.7}
        ...
        <div className={`question-card-overlay ${showQuestion ? 'show' : ''}`}>
          <div className="question-card">
              ...
              {question.answers.map((answer, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="answer-text">{answer.text}</span>
                    {showResult && isCorrect && <span className="answer-icon">✓</span>}
                    {showResult && isSelected && !isCorrect && <span className="answer-icon">✗</span>}
                  </button>
                );
              })}
            </div>
            ...
            {showResult && (
              <div className={`result-inline ${answeredCorrectly ? 'correct-result' : 'incorrect-result'}`}>
                <div className="result-header">
                  <span className="result-icon">{answeredCorrectly ? '🎉' : '💡'}</span>
                  <span className="result-title">{answeredCorrectly ? 'Correct!' : 'Not quite!'}</span>
                </div>
                <p className="result-explanation">{question.explanation}</p>
                <button className="next-compact-button" onClick={handleNextQuestion}>
                  {currentQuestion < GAME_QUESTIONS.length - 1 ? <>Next →</> : <>Results 🏆</>}
                </button>
              </div>
            )}
          </div>
