  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (currentView === 'register') {
        ...
        setCurrentView('login'); // بعد الريجيست يوديه اللوجين
        return;
      }
...
//       const data = await response.json();
//       // المتوقع من الباك: { token, user: {...} }
//       localStorage.setItem("token", data.token);
//       localStorage.setItem("user", JSON.stringify(data.user));

//       alert(currentView === 'login'
//         ? `Login successful! Welcome back ${data.user.fullName || ''}`
...
//   return (
//     <div className="rl-container" style={{ backgroundImage: `url(${Background})` }}>
...
//           <div className="rl-logo">
//             <Telescope className="rl-icon" />
//             <h1 className="rl-title">NASA Eyes</h1>
//           </div>
//           <p className="rl-subtitle">
//             {currentView === 'login' ? 'Welcome back, explorer!' : 'Join the space exploration community'}
//           </p>
